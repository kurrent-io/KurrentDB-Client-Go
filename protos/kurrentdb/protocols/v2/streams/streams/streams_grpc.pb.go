...
const (
	...
	StreamsService_AppendRecords_FullMethodName = "/kurrentdb.protocol.v2.streams.StreamsService/AppendRecords"
)
...
type StreamsServiceClient interface {
	...
	AppendRecords(ctx context.Context, in *AppendRecordsRequest, opts ...grpc.CallOption) (*AppendRecordsResponse, error)
}
...
func (c *streamsServiceClient) AppendRecords(ctx context.Context, in *AppendRecordsRequest, opts ...grpc.CallOption) (*AppendRecordsResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(AppendRecordsResponse)
	err := c.cc.Invoke(ctx, StreamsService_AppendRecords_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type StreamsServiceServer interface {
	...
	// On consistency check failure, no records are written and all failing checks
	// are reported in the response so the client can refresh stale state in one round trip.
	AppendRecords(context.Context, *AppendRecordsRequest) (*AppendRecordsResponse, error)
...
func (UnimplementedStreamsServiceServer) AppendSession(grpc.ClientStreamingServer[AppendRequest, AppendSessionResponse]) error {
	return status.Error(codes.Unimplemented, "method AppendSession not implemented")
}
func (UnimplementedStreamsServiceServer) AppendRecords(context.Context, *AppendRecordsRequest) (*AppendRecordsResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method AppendRecords not implemented")
}
...
func _StreamsService_AppendRecords_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(AppendRecordsRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(StreamsServiceServer).AppendRecords(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: StreamsService_AppendRecords_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(StreamsServiceServer).AppendRecords(ctx, req.(*AppendRecordsRequest))
	}
	...
	Methods: []grpc.MethodDesc{
		{
			...
			Handler:    _StreamsService_AppendRecords_Handler,
		},
	},
