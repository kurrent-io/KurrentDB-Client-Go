...
import (
	...
	gossipApi "github.com/EventStore/EventStore-Client-Go/v1/protos/gossip"
	server_features "github.com/EventStore/EventStore-Client-Go/v1/protos/serverfeatures"
	"github.com/EventStore/EventStore-Client-Go/v1/protos/shared"
...
func getSupportedMethods(ctx context.Context, conf *Configuration, conn *grpc.ClientConn) (*serverInfo, error) {
	...
	for idx, value := range strings.Split(methods.EventStoreServerVersion, ".") {
		...
		if err != nil {
			return nil, fmt.Errorf("invalid KurrentDB server version format: %w", err)
		}
