...
func SubscriptionTests(t *testing.T, emptyDBClient *kurrentdb.Client, populatedDBClient *kurrentdb.Client) {
...
func subscriptionToStreamCaughtUpMessage(db *kurrentdb.Client) TestCall {
	...
	return func(t *testing.T) {
		...
		subscription, err := db.SubscribeToStream(ctx, streamID, kurrentdb.SubscribeToStreamOptions{From: kurrentdb.Start{}})
...
func streamSubscriptionDeliversAllEventsInStreamAndListensForNewEvents(db *kurrentdb.Client) TestCall {
	return func(t *testing.T) {
		if db == nil {
			t.Skip()
		}
		...
		subscription, err := db.SubscribeToStream(context.Background(), "dataset20M-0", kurrentdb.SubscribeToStreamOptions{
...
func allSubscriptionWithFilterDeliversCorrectEvents(db *kurrentdb.Client) TestCall {
	return func(t *testing.T) {
		if db == nil {
			t.Skip()
		}
		...
		subscription, err := db.SubscribeToAll(context.Background(), kurrentdb.SubscribeToAllOptions{
			From: kurrentdb.Start{},
...
func subscriptionAllFilter(db *kurrentdb.Client) TestCall {
	return func(t *testing.T) {
		sub, err := db.SubscribeToAll(context.Background(), kurrentdb.SubscribeToAllOptions{From: kurrentdb.Start{}, Filter: kurrentdb.ExcludeSystemEventsFilter()})
...
func connectionClosing(db *kurrentdb.Client) TestCall {
	return func(t *testing.T) {
		if db == nil {
			t.Skip()
		}
...
func subscriptionAllWithCredentialsOverride(db *kurrentdb.Client) TestCall {
	return func(t *testing.T) {
		if db == nil {
			t.Skip()
		}
