...
func SubscriptionTests(t *testing.T, emptyDBClient *kurrent.Client, populatedDBClient *kurrent.Client) {
...
func subscriptionToStreamCaughtUpMessage(db *kurrent.Client) TestCall {
	...
	return func(t *testing.T) {
		...
		subscription, err := db.SubscribeToStream(ctx, streamID, kurrent.SubscribeToStreamOptions{From: kurrent.Start{}})
...
func streamSubscriptionDeliversAllEventsInStreamAndListensForNewEvents(db *kurrent.Client) TestCall {
	return func(t *testing.T) {
		if db == nil {
			t.Skip()
		}
		...
		subscription, err := db.SubscribeToStream(context.Background(), "dataset20M-0", kurrent.SubscribeToStreamOptions{
			From: kurrent.Start{},
		})
		...
		// Write a new event
		opts2 := kurrent.AppendToStreamOptions{
			ExpectedRevision: kurrent.Revision(5_999),
		}
...
func allSubscriptionWithFilterDeliversCorrectEvents(db *kurrent.Client) TestCall {
	return func(t *testing.T) {
		if db == nil {
			t.Skip()
		}

		positionsContent, err := os.ReadFile("../resources/test/all-positions-filtered-stream-194-e0-e30.json")
		require.NoError(t, err)
		versionsContent, err := os.ReadFile("../resources/test/all-versions-filtered-stream-194-e0-e30.json")
		require.NoError(t, err)
		...
		subscription, err := db.SubscribeToAll(context.Background(), kurrent.SubscribeToAllOptions{
			From: kurrent.Start{},
...
func subscriptionAllFilter(db *kurrent.Client) TestCall {
	return func(t *testing.T) {
		sub, err := db.SubscribeToAll(context.Background(), kurrent.SubscribeToAllOptions{From: kurrent.Start{}, Filter: kurrent.ExcludeSystemEventsFilter()})
...
func connectionClosing(db *kurrent.Client) TestCall {
	return func(t *testing.T) {
		if db == nil {
			t.Skip()
		}
...
func subscriptionAllWithCredentialsOverride(db *kurrent.Client) TestCall {
	return func(t *testing.T) {
		if db == nil {
			t.Skip()
		}

		opts := kurrent.SubscribeToAllOptions{
