...
result, err := db.AppendToStream(context.Background(), "orders-123", options, esdb.EventData{
...
```go{7-12}
_, err = db.AppendToStream(context.Background(), "orders-456", esdb.AppendToStreamOptions{}, event)
...
_, err = db.AppendToStream(context.Background(), "order-123", options, esdb.EventData{
    ContentType: esdb.ContentTypeJson,
    EventType:   "OrderPlaced",
    Data:        bytes,
})
...
- `esdb.Any` - No concurrency check
- `esdb.NoStream{}` - Stream should not exist
- `esdb.StreamExists{}` - Stream should exist
- `esdb.StreamRevision{}` - Stream should be at specific revision
...
_, err = db.AppendToStream(context.Background(), "orders-123", aopts, esdb.EventData{
    ContentType: esdb.ContentTypeJson,
    EventType:   "OrderPlaced",
    Data:        bytes,
})
